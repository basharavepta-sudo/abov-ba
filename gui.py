...
class StyledButton(tk.Canvas):
    ...
    def _draw(self):
        ...
        if self.focused and not self.disabled:
            self._rounded_border(1, 1, self.width-1, self.height-1, 7, Theme.ACCENT_GLOW, 2)
    ...
    def _rounded_border(self, x1, y1, x2, y2, r, color, width=1):
        self.create_arc(x1, y1, x1+2*r, y1+2*r, start=90, extent=90, style="arc", outline=color, width=width)
        self.create_arc(x2-2*r, y1, x2, y1+2*r, start=0, extent=90, style="arc", outline=color, width=width)
        self.create_arc(x1, y2-2*r, x1+2*r, y2, start=180, extent=90, style="arc", outline=color, width=width)
        self.create_arc(x2-2*r, y2-2*r, x2, y2, start=270, extent=90, style="arc", outline=color, width=width)
